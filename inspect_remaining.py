...
def inspect_remaining():
    ...
    cur.execute(
        "SELECT COUNT(*) FROM clients_clientprofile WHERE user_id NOT IN (SELECT id FROM users_user)"
    )
    print(f"Orphan profiles (no users_user): {cur.fetchone()[0]}")

    # Redundancy check: is supabase_user_id always same as user_id?
    cur.execute(
        "SELECT COUNT(*) FROM chefs_chefprofile WHERE user_id != supabase_user_id AND supabase_user_id IS NOT NULL"
    )
    print(
        f"Chef profiles with mismatched user_id/supabase_user_id: {cur.fetchone()[0]}"
    )
