...
class Refund(models.Model):
    ...
    refund_type = models.CharField(
        max_length=20, choices=RefundType.choices, default=RefundType.FULL
    )
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, validators=[MinValueValidator(0.01)]
    )
...
class ChefPayout(models.Model):
    ...
    booking = models.ForeignKey(
        Booking, on_delete=models.CASCADE, related_name="chef_payouts"
    )
    payment = models.ForeignKey(
        Payment, on_delete=models.CASCADE, related_name="chef_payouts"
    )
    ...
    def __str__(self):
        return (
            f"Payout #{self.id} - {self.chef.full_name} - {self.amount} {self.currency}"
        )
...
class MpesaPayment(models.Model):
    ...
    payment = models.OneToOneField(
        Payment, on_delete=models.CASCADE, related_name="mpesa_payment"
    )
