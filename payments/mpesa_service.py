...
class MpesaService:
    ...
    def initiate_stk_push(
        self, phone_number, amount, account_reference, transaction_desc
    ):
    ...
    def query_stk_status(self, checkout_request_id):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {"success": False, "error": "Status query failed"}
    ...
    def process_booking_payment(self, booking_id, phone_number):
        ...
        try:
            ...
            existing_payment = Payment.objects.filter(
                booking=booking, status="pending", payment_method="mpesa"
            ).first()

            if existing_payment:
                # Check if there's a corresponding M-Pesa record
                mpesa_record = MpesaPayment.objects.filter(
        ...
        try:
            ...
            if status_result["success"]:
                ...
                if result_code == "0":  # Success
                    ...
                    mpesa_payment.mpesa_receipt_number = status_result["data"].get(
                        "MpesaReceiptNumber", ""
                    )
                    mpesa_payment.save()
            ...
            return {"success": False, "error": "M-Pesa payment not found"}
        except Exception as e:
    ...
    def handle_callback(self, callback_data):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"M-Pesa payment not found for checkout request: {checkout_request_id}"
                )
            ...
            if not verification["success"]:
                logger.warning(
                    f"Could not verify payment status with M-Pesa: {verification.get('error')}"
                )
            ...
            if verification.get("result_code") != "0":
                logger.error(
                    f"Payment verification returned non-success: {verification.get('result_code')}"
                )
