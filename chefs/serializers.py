...
class ChefReviewSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Update chef stats
        chef = review.chef
        ratings = ChefReview.objects.filter(chef=chef).values_list("rating", flat=True)
        if ratings:
            chef.average_rating = sum(ratings) / len(ratings)
            chef.total_reviews = len(ratings)
        chef.total_bookings = Booking.objects.filter(
            chef=chef, status=Booking.Status.COMPLETED
        ).count()
        chef.save()
    ...
    def validate_food_quality(self, value):
        if value is not None and (value < 1 or value > 5):
            raise serializers.ValidationError(
                "Food quality rating must be between 1 and 5"
            )
        return value
    ...
    def validate_professionalism(self, value):
        if value is not None and (value < 1 or value > 5):
            raise serializers.ValidationError(
                "Professionalism rating must be between 1 and 5"
            )
        return value
...
class ChefSearchSerializer(serializers.Serializer):
    ...
    min_hourly_rate = serializers.DecimalField(
        max_digits=8, decimal_places=2, required=False, min_value=0
    )
    experience_level = serializers.ChoiceField(
        choices=ChefProfile.ExperienceLevel.choices, required=False
    )
