...
class GeminiAIServiceTestCase(TestCase):
    ...
    def setUp(self):
        # Create a mock for the model first to avoid real initialization
        with patch("google.generativeai.configure"), \
            ...
            # Ensure the service uses our mock instance
    ...
    def test_get_chef_recommendations_success(self):
        ...
        """
        with patch.object(self.ai_service.model, 'generate_content', return_value=mock_response):
    ...
    def test_get_chef_recommendations_failure(self):
        """Test chef recommendations with API failure"""
        with patch.object(self.ai_service.model, 'generate_content', side_effect=Exception("API Error")):
            result = self.ai_service.get_chef_recommendations(
    ...
    def test_get_menu_suggestions_success(self):
    ...
    def test_chat_with_ai_success(self):
        ...
        mock_response = Mock()
        mock_response.text = (
            "I'd be happy to help you find the perfect chef for your event!"
        )
        with patch.object(self.ai_service.model, 'generate_content', return_value=mock_response):
    ...
    def test_analyze_chef_profile_success(self):
    ...
    def test_generate_meal_plan_success(self):
        ...
        with patch.object(self.ai_service.model, 'generate_content', return_value=mock_response):
            result = self.ai_service.generate_meal_plan(
                dietary_goals="Healthy, high protein",
                duration_days=7,
    ...
    def test_generate_cooking_tips_success(self):
        ...
        with patch.object(self.ai_service.model, 'generate_content', return_value=mock_response):
            result = self.ai_service.generate_cooking_tips(
                skill_level="beginner", cuisine_type="Italian", specific_dish="pasta"
            )
...
class AIAPITestCase(APITestCase):
    ...
    def test_chef_recommendation_request_success(self, mock_recommend):
        ...
        response = self.client.post(
            "/api/ai/recommendations/chefs/", data, format="json"
        )
    ...
    def test_menu_recommendation_request_success(self, mock_suggest):
        ...
        response = self.client.post(
            "/api/ai/recommendations/menus/", data, format="json"
        )
    ...
    def test_chat_session_creation(self):
        ...
        data = {"session_name": "Wedding Planning Chat"}

        response = self.client.post(
            "/api/ai/chat/sessions/create/", data, format="json"
        )
    ...
    def test_chat_session_list(self):
        ...
        ChatSession.objects.create(user=self.user, session_name="Test Session")
    ...
    def test_chat_message_creation(self, mock_chat):
        ...
        session = ChatSession.objects.create(
            user=self.user, session_name="Test Session"
        )
        ...
        response = self.client.post(
            "/api/ai/chat/messages/create/", data, format="json"
        )
...
class AIRecommendationModelTestCase(TestCase):
    ...
    def test_chat_session_creation(self):
        ...
        session = ChatSession.objects.create(
            user=self.user, session_name="Wedding Planning"
        )
    ...
    def test_chat_message_creation(self):
        ...
        message = ChatMessage.objects.create(
            session=session, message="Hello, I need help", message_type="user"
        )
