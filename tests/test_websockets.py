...
class ChatConsumerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.chef_profile = ChefProfile.objects.create(
            user=self.chef_user, bio="Test chef", experience_years=5
        )

        self.chat_room = ChatRoom.objects.create(
            client=self.client_user, chef=self.chef_profile
        )

    async def test_chat_consumer_connect_authenticated(self):
        """Test WebSocket connection with authenticated user"""
        communicator = WebsocketCommunicator(
            ChatConsumer.as_asgi(), f"/ws/chat/{self.chat_room.id}/"
        )

        # Mock authentication and URL route
        communicator.scope["user"] = self.client_user
        communicator.scope["url_route"] = {"kwargs": {"room_id": str(self.chat_room.id)}}
    ...
    async def test_chat_consumer_connect_unauthenticated(self):
        ...
        # Mock anonymous user and URL route
    ...
    async def test_chat_consumer_typing_indicator(self):
        ...
        typing_data = {"type": "typing", "is_typing": True}
    ...
    async def test_chat_consumer_read_receipt(self):
        ...
        # Send read receipt
        read_receipt_data = {"type": "read_receipt", "message_id": message.id}
...
class NotificationConsumerTestCase(TestCase):
    ...
    async def test_notification_consumer_connect_authenticated(self):
        ...
        communicator = WebsocketCommunicator(
            NotificationConsumer.as_asgi(), f"/ws/notifications/{self.user.id}/"
        )
    ...
    async def test_notification_consumer_connect_wrong_user(self):
        ...
        communicator = WebsocketCommunicator(
            NotificationConsumer.as_asgi(), f"/ws/notifications/{other_user.id}/"
        )

        # Try to connect as different user
        communicator.scope["user"] = self.user
        communicator.scope["url_route"] = {"kwargs": {"user_id": str(other_user.id)}}
    ...
    async def test_notification_consumer_mark_read(self):
        ...
        mark_read_data = {"type": "mark_read", "notification_id": 123}
...
class WebSocketIntegrationTestCase(TestCase):
    ...
    async def test_multi_user_chat(self):
        ...
        client_communicator.scope["url_route"] = {"kwargs": {"room_id": str(self.chat_room.id)}}
        ...
        chef_communicator.scope["url_route"] = {"kwargs": {"room_id": str(self.chat_room.id)}}
