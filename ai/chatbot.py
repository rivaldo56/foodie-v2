...
Always be helpful and guide users toward making a booking!"""
    ...
    def _detect_intent(self, message):
        ...
        if any(
            word in message_lower for word in ["book", "booking", "reserve", "schedule"]
        ):
            return "booking"
        elif any(
            word in message_lower
            for word in ["recommend", "suggest", "what should", "help me find"]
        ):
    ...
    def _extract_booking_data(self, user_message, ai_response):
        ...
        if "personal meal" in message_lower or "dinner" in message_lower:
            ...
            "catering" in message_lower
            or "event" in message_lower
            or "party" in message_lower
        ):
    ...
    def get_meal_recommendations(
        self, dietary_requirements=None, cuisine_preference=None, budget=None
    ):
...
Provide 3-5 specific meal recommendations with brief explanations of why each is a good fit."""
            ...
            return {
                ...
                "menu_items": list(
                    menu_items.values("id", "name", "price_per_serving", "description")[
                        :5
                    ]
                ),
            }
    ...
    def get_chef_recommendations(
        self, service_type=None, location=None, budget=None, cuisine=None
    ):
