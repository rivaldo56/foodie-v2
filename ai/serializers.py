...
class AIRecommendationRequestSerializer(serializers.Serializer):
    ...
    recommendation_type = serializers.ChoiceField(
        choices=AIRecommendation.RECOMMENDATION_TYPE_CHOICES
    )
...
class ChatMessageCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_session(self, value):
        ...
        if value.user != user:
            raise serializers.ValidationError(
                "You can only send messages to your own chat sessions"
            )
...
class ChatMessageAPISerializer(serializers.Serializer):
    ...
    message_type = serializers.ChoiceField(
        choices=["user", "ai", "system"], default="user"
    )
    ...
    def create(self, validated_data):
        ...
        user_message = ChatMessage.objects.create(
            session=session, sender=message_type, content=message_content
        )
...
class MenuRecommendationSerializer(serializers.Serializer):
    ...
    budget_per_person = serializers.DecimalField(
...
class ChefRecommendationSerializer(serializers.Serializer):
    ...
    experience_level = serializers.ChoiceField(
        choices=[("any", "Any")]
        + list(ChefProfileSerializer.Meta.model.ExperienceLevel.choices),
        required=False,
        default="any",
    )
