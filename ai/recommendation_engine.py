...
class RecommendationEngine:
    ...
    def _get_chef_recommendations(self, limit):
        ...
        user_favorites = set(
            FavoriteChef.objects.filter(user=self.user).values_list(
                "chef_id", flat=True
            )
        )
        user_bookings = set(
            Booking.objects.filter(client=self.user).values_list("chef_id", flat=True)
        )
        ...
        # Merge with ClientProfile data (initial preferences)
        from users.models import ClientProfile
        try:
            profile = ClientProfile.objects.get(user=self.user)
            if profile.preferred_cuisines:
                preferred_cuisines.update(profile.preferred_cuisines)
            if profile.budget_range and not price_range:
                # Map budget_range string to numeric range if needed
                # For now, just use a heuristic if price_range is empty
                if profile.budget_range == "Budget":
                    price_range = {"min": 0, "max": 3000}
                elif profile.budget_range == "Fair Market":
                    price_range = {"min": 3000, "max": 7000}
                elif profile.budget_range == "Premium":
                    price_range = {"min": 7000, "max": 20000}
        ...
        candidate_chefs = (
            ChefProfile.objects.filter(is_available=True)
            ...
            .order_by("-average_rating", "-created_at")[:100]
        )  # Limit pool size
        ...
        for chef in candidate_chefs:
            try:
                ...
                if similar_users:
                    ...
                    fav_count = sum(
                        1
                        for fc in chef.favorited_by.all()
                        if fc.user_id in similar_users
                    )
                    book_count = sum(
                        1 for b in chef.bookings.all() if b.client_id in similar_users
                    )
                ...
                popularity_score = (rating_score * 0.6) + (booking_score * 0.4)
    ...
    def track_interaction(self, content_type, content_id, interaction_type):
        ...
        weights = {"view": 1, "like": 3, "book": 5, "share": 4}
    ...
    def seed_preferences_from_onboarding(self, onboarding_data):
        """Seed learned preferences from onboarding data upon completion"""
        ...
        try:
            ...
            # Seed cuisines
            cuisines = set(user_prefs.preferred_cuisines or [])
            if onboarding_data.preferred_cuisines:
                cuisines.update(onboarding_data.preferred_cuisines)
            user_prefs.preferred_cuisines = list(cuisines)

            # Seed budget as initial price range
            if onboarding_data.budget_range and not user_prefs.preferred_price_range:
                if onboarding_data.budget_range == "Budget":
                    user_prefs.preferred_price_range = {"min": 0, "max": 3000}
                elif onboarding_data.budget_range == "Fair Market":
                    user_prefs.preferred_price_range = {"min": 3000, "max": 7000}
                elif onboarding_data.budget_range == "Premium":
                    user_prefs.preferred_price_range = {"min": 7000, "max": 20000}

            user_prefs.confidence_level = 0.3  # Initial confidence from self-reported data
            user_prefs.save()

            # Clear cache to ensure immediate personalization
            from django.core.cache import cache
            cache.delete(f"personalized_feed_{self.user.id}")

        except Exception as e:
            logger.error(f"Error seeding preferences: {str(e)}")
        ...
        chefs = (
            ...
            .annotate(
                recent_bookings=Count(
                    "booking", filter=Q(booking__created_at__gte=last_24h)
                ),
