...
class ChatbotMessageView(generics.CreateAPIView):
    ...
    def post(self, request):
        ...
        if session_id:
            try:
                ...
                session = ChatSession.objects.create(
                    user=request.user, session_type="booking_assistance"
                )
        else:
            session = ChatSession.objects.create(
                user=request.user, session_type="booking_assistance"
        ...
        context = {
            ...
            "previous_bookings": (
                request.user.bookings.count()
                if hasattr(request.user, "bookings")
                else 0
            ),
        }
...
class ChefRecommendationAIView(generics.CreateAPIView):
    ...
    def post(self, request):
        ...
        result = chatbot.get_chef_recommendations(
            service_type=service_type, location=location, budget=budget, cuisine=cuisine
        )
...
class DietaryAnalysisView(generics.CreateAPIView):
    ...
    def post(self, request):
        ...
        if result["success"]:
            return Response({"analysis": result["analysis"]}, status=status.HTTP_200_OK)
        else:
            return Response(
