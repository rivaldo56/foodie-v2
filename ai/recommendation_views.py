                ...
                results.append(
                    {
                        ...
                        "score_breakdown": {
                            "collaborative": round(
                                item["breakdown"]["collaborative"], 3
                            ),
...
class TrackInteractionView(generics.CreateAPIView):
    ...
    def post(self, request):
        ...
        interaction_type = request.data.get(
            "interaction_type"
        )  # 'view', 'like', 'book', 'share', 'skip'
...
class SimilarChefsView(generics.GenericAPIView):
    ...
    def get(self, request, chef_id):
        ...
        for similar_chef in similar_chefs:
            if chef.specialties and similar_chef.specialties:
                ...
                if overlap > 0:
                    ...
                    # Price similarity
                    price_diff = abs(
                        float(chef.hourly_rate) - float(similar_chef.hourly_rate)
                    )
                    price_score = 1.0 / (1.0 + price_diff / 1000)  # Normalize

                    # Rating similarity
                    rating_diff = abs(
                        float(chef.average_rating) - float(similar_chef.average_rating)
                    )
                    rating_score = 1.0 - (rating_diff / 5.0)

                    # Combined score
                    total_score = (
                        specialty_score * 0.5 + price_score * 0.3 + rating_score * 0.2
                    )

                    scored_chefs.append(
                        {"chef": similar_chef, "similarity_score": total_score}
                    )
        ...
        return Response(
            {"similar_chefs": results, "total": len(results)}, status=status.HTTP_200_OK
        )
