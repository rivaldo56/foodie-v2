...
export default function BookingPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const [menuRes, mealsRes, userRes] = await Promise.all([
          api.get(`/experiences/menus/${menuId}/`),
        ...
        if (menuRes.data) {
          const minGuests = menuRes.data.min_guests || 2;
  ...
  const handleBookingFlow = async (e: React.FormEvent) => {
    ...
    try {
      console.log('[BOOKING_SUBMIT] Invoking create-booking API...');
      
      const res = await api.post('/bookings/create/', {
        v4_menu_id: Number(menuId),
      ...
      console.log('[BOOKING_SUBMIT] Success, redirecting to Paystack:', res.data.authorization_url);
