...
export default function MealBookingPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Check auth status
        const authRes = await api.get('/api/auth/me/').catch(() => null);
        setUser(authRes?.data || null);

        // Fetch meal (MenuItem)
        const res = await api.get(`/api/bookings/menu-items/${mealId}/`);
        const data = res.data;
        setMeal({
            id: data.id,
            name: data.name,
            description: data.description,
            price: parseFloat(data.price_per_serving),
            kcal: data.calories,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('[BOOKING_SUBMIT] Creating booking via Django API...');

      const response = await api.post('/api/bookings/create/', {
          meal_id: mealId,
          booking_date: new Date(formData.date_time).toISOString(),
          service_address: formData.address,
          number_of_guests: Number(formData.guests_count),
          special_requests: formData.special_requests,
          items: [{ menu_item: mealId, quantity: formData.guests_count }] // For serializers that expect items
      });

      const result = response.data;

      if (!result?.authorization_url) {
        console.warn('[BOOKING_SUBMIT] No authorization_url returned. Response:', result);
