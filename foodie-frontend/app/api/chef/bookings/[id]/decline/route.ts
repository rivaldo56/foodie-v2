...
export async function POST(
  ...
  try {
    ...
    // 2. Call the respond action with 'declined'
    const respondRes = await fetch(`${API_BASE_URL}/bookings/api/assignments/${pendingAssignment.id}/respond/`, {
        ...
        body: JSON.stringify({
            response: 'declined',
            reason: body.reason || 'Declined by chef via mobile/web interface',
        }),
    });
    ...
    if (!respondRes.ok) {
        return NextResponse.json({ error: result.detail || 'Failed to decline booking' }, { status: respondRes.status });
    }
