...
import { LayoutDashboard, ShoppingBasket, ClipboardList, UserRound } from 'lucide-react';

const FARMER_LINKS = [
    { href: '/farmer/dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { href: '/farmer/products', label: 'My Farm', icon: ShoppingBasket },
    ...
    return (
        <nav className="fixed inset-x-0 bottom-0 z-40 flex justify-center bg-black/50 pb-4 pt-2 backdrop-blur">
            <div className="flex w-full max-w-xl items-center justify-between gap-2 rounded-full border border-white/10 bg-black/70 px-4 py-2 text-xs font-medium text-white/70">
                {FARMER_LINKS.map(({ href, label, icon: Icon }) => {
                    ...
                    return (
                        ...
                        >
                            <span
                                className={`inline-flex h-10 w-10 items-center justify-center rounded-full transition ${active ? 'bg-green-600 text-white shadow-glow' : 'bg-white/10 text-white/70'
                                    }`}
                            >
