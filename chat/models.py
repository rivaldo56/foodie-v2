...
class ChatRoom(models.Model):
    ...
    client = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="client_chat_rooms"
    )
    chef = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="chef_chat_rooms"
    )
...
class Message(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.sender.full_name}: {self.content[:50]}..."
            if self.content
            else f"{self.sender.full_name}: [{self.message_type}]"
        )
...
class Notification(models.Model):
    """
    General purpose notifications for users.
    """

    class NotificationType(models.TextChoices):
        BOOKING_UPDATE = "booking_update", _("Booking Update")
