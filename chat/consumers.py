...
def _get_user_id_from_scope(scope):
    """Extraction of user_id from authenticated scope."""
    user = scope.get("user")
    if user and getattr(user, "is_authenticated", False):
        return getattr(user, "id", None)
    
    # Strictly for development/test fallback if needed, but preferred to use user scope
    return None


class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        # Room ID should come from URL route, sanitized by middleware
        ...
        await self.channel_layer.group_send(
            ...
            {"type": "user_join", "user": self.user.username, "user_id": self.user.id},
        )
...
class NotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        url_kwargs = self.scope.get("url_route", {}).get("kwargs", {})
        self.target_user_id = url_kwargs.get("user_id")
        self.user = self.scope["user"]
        ...
        if (
            ...
            or str(self.user.id) != str(self.target_user_id)
        ):
            await self.close()
            return

        self.notification_group_name = f"notifications_{self.target_user_id}"
