    ...
    def validate(self, attrs):
        ...
        if message_type == "image" and not attrs.get("image_attachment"):
            raise serializers.ValidationError(
                "Image messages must have an image attachment"
            )
...
class ChatRoomCreateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not booking_id and not chef_id:
            raise serializers.ValidationError(
                "Either booking_id or chef_id must be provided"
            )

        if booking_id and chef_id:
            raise serializers.ValidationError(
                "Provide either booking_id or chef_id, not both"
            )
    ...
    def validate_booking_id(self, value):
        ...
        try:
            ...
            if user != booking.client and user != booking.chef.user:
                raise serializers.ValidationError(
                    "You are not involved in this booking"
                )
    ...
    def create(self, validated_data):
        ...
        if validated_data.get("booking_id"):
            ...
            chat_room = ChatRoom.objects.create(
                booking=booking, client=booking.client, chef=booking.chef.user
            )
            return chat_room
            ...
            chat_room, created = ChatRoom.objects.get_or_create(
                client=user, chef=chef_user, booking=None
            )
            return chat_room
