...
class AdminDashboardView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        month_revenue = Booking.objects.filter(
            status__in=["completed", "confirmed"], created_at__gte=month_start
        ).aggregate(total=Sum("total_amount"))["total"] or Decimal("0.00")

        # Average ratings
        avg_platform_rating = (
            ChefReview.objects.aggregate(avg=Avg("rating"))["avg"] or 0
        )
        ...
        for i in range(30):
            ...
            daily_signups.append({"date": day.strftime("%Y-%m-%d"), "count": signups})

            daily_bookings.append({"date": day.strftime("%Y-%m-%d"), "count": bookings})
        ...
        return Response(
            {
                ...
                "ratings": {"average_platform_rating": round(avg_platform_rating, 2)},
            },
...
class AdminUserListView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"users": users_data, "total": users.count()}, status=status.HTTP_200_OK
        )
...
class AdminUserDetailView(generics.GenericAPIView):
    ...
    def patch(self, request, user_id):
        ...
        return Response(
            {"message": "User updated successfully", "is_active": user.is_active},
            status=status.HTTP_200_OK,
        )
...
class AdminChefVerificationView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"pending_chefs": chefs_data, "total": pending_chefs.count()},
            status=status.HTTP_200_OK,
        )
    ...
    def post(self, request, chef_id):
        ...
        try:
            ...
            return Response(
                {"error": "Chef not found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if action == "verify":
            ...
            chef.save()
            return Response(
                {"message": "Chef verified successfully"}, status=status.HTTP_200_OK
            )
        elif action == "reject":
            # In a real app, you might want to send an email or keep a record
            return Response(
                {"message": "Chef verification rejected"}, status=status.HTTP_200_OK
            )
        else:
            return Response(
...
class AdminBookingOversightView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"bookings": bookings_data, "total": bookings.count()},
            status=status.HTTP_200_OK,
        )
